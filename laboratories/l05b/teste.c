...
void getInstData(char inst[], InstData *data) {
    ...
    if (strcmpCustom(inst, "lui", 3) == 0) {
        ...
        r1_r2_imm(inst, &rd, &rs1, &imm, 4);
        opcode = 19, imm = imm%32, funct3 = 1, type = I2;
    } 
    else if (strcmpCustom(inst, "srli ", 5) == 0) {
        // srli rd, rs1, shamt
        // OPCODE = 0010011 = 19 FUNCT3 = 5
        r1_r2_imm(inst, &rd, &rs1, &imm, 4);
        opcode = 19, imm = imm%32, funct3 = 5, type = I2;
    } 
    else if (strcmpCustom(inst, "srai ", 5) == 0) {
        // srai rd, rs1, shamt
        // OPCODE = 0010011 = 19 FUNCT3 = 5
        r1_r2_imm(inst, &rd, &rs1, &imm, 4);
        opcode = 19, imm = imm%32 + 1024, funct3 = 5, funct7 = 32, type = I2;
    } 
...
void pack(InstData *data, int packed_bin[]) {
    ...
    if (data->type == R) {
        ...
        copyString(tmp_imm, packed_bin, 11, 5, 11);
    }
    else if (data->type == B) {
...
int main() {
    ...
    getInstData(input, &inst_data);

    int packed_bin[33], packed_int;

    pack(&inst_data, packed_bin);
    for(int i = 0; i < 32; i++)
    ...
    /*
    000000100110110100010000011000100011
    00000001001100010000011000100011
    */
